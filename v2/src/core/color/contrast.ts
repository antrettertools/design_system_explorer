...
export interface ContrastResult {
  ratio: number; ratioDisplay: string
  aaBodyText: boolean; aaLargeText: boolean; aaaBodyText: boolean; aaaLargeText: boolean
...
export function checkContrast(fg: string, bg: string): ContrastResult {
  ...
  return { ratio, ratioDisplay: ratio.toFixed(2), ...levels, level }
}

export interface ContrastEntry {
  hex: string; label: string; onWhite: ContrastResult; onBlack: ContrastResult
}

export function buildContrastEntries(colors: Array<{ hex: string; label: string }>): ContrastEntry[] {
