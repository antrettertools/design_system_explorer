...
export function TypeSpecimen({ customText }: TypeSpecimenProps) {
  ...
  const ts = useMemo(
    () => generateTypeScale(typography.fontSize, typography.scaleAlgorithm, typography.modularRatio),
    [typography.fontSize, typography.scaleAlgorithm, typography.modularRatio],
  )
  ...
  if (customText !== undefined) {
    return (
      <div style={{ ...bodyStyle(ts.base), color: 'var(--chrome-text)' }}>
        {customText || <span style={{ color: 'var(--chrome-text-subtle)' }}>Type something…</span>}
      </div>
    )
  }
        ...
        return (
          ...
          >
            <div style={{
              ...
              color: isHeading ? 'var(--chrome-accent)' : '#6366f1',
              paddingTop: 2,
            }}>
              {info.role}
            </div>
            ...
            <div style={{
              ...
              <div style={{ color: isHeading ? 'var(--chrome-accent)' : '#6366f180', fontSize: 8 }}>
                {step}
              </div>
            </div>
          </div>
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
      <span style={{
        ...
        padding: '2px 6px',
        borderRadius: 4,
        background: isH ? 'rgba(232,168,48,0.15)' : '#6366f118',
        color: isH ? 'var(--chrome-accent)' : '#6366f1',
      }}>
        {badge}
      </span>
      <span style={{ fontSize: 11, color: 'var(--chrome-text-subtle)' }}>{role}</span>
      <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--chrome-text)' }}>{family}</span>
    </div>
  )
}
