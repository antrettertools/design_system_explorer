...
import { generateSpacingScale, generateRadiusScale, DEFAULT_Z_INDEX } from '@/core/spacing/scale'
...
export function SpacingPanel() {
  ...
  const spacingScale = useMemo(
    () => generateSpacingScale(spacing.base, spacing.algorithm),
    [spacing.base, spacing.algorithm],
  )

  const radiusScale = useMemo(
    () => generateRadiusScale(spacing.radiusBase, spacing.radiusScale),
    [spacing.radiusBase, spacing.radiusScale],
  )

  const steps = Object.entries(spacingScale)
    ...
    <>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {steps.map(([key, val]) => (
            <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ fontFamily: 'var(--chrome-font-mono)', fontSize: 11, color: 'var(--chrome-accent)', width: 100, flexShrink: 0 }}>
                --{key}
              </div>
              ...
              <div style={{ fontSize: 11, color: 'var(--chrome-text-subtle)', width: 48, textAlign: 'right', flexShrink: 0 }}>
                {val}px
              </div>
            </div>
          ))}
      ...
      <PreviewCard title="Border Radius Scale">
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, alignItems: 'flex-end' }}>
          {Object.entries(radiusScale).map(([key, val]) => (
            <div key={key} style={{ textAlign: 'center' }}>
              ...
              />
              <div style={{ fontSize: 10, color: 'var(--chrome-text-subtle)', fontFamily: 'var(--chrome-font-mono)' }}>
                --radius-{key}
                <br />
      ...
      <PreviewCard title="Z-Index Scale">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {Object.entries(DEFAULT_Z_INDEX).map(([key, val]) => (
            <div key={key} style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
              <div style={{ fontFamily: 'var(--chrome-font-mono)', fontSize: 11, color: 'var(--chrome-accent)', width: 120 }}>
                --z-{key}
              </div>
              <div style={{ fontSize: 11, color: 'var(--chrome-text-subtle)' }}>{val}</div>
            </div>
          ))}
        </div>
      </PreviewCard>
